...
app.layout = html.Div([
    ...
    html.H1("Interactive Plot with Dash"),

    html.P('this is a dash example !!'),

    #dropdown menu for selecting specific datasets
    dcc.Dropdown(
        id = 'data-selector', #dropdown menu id
        options = [
            {'label': 'Dataset 1', 'value': 'dataset1'},
            {'label': 'Dataset 2', 'value': 'dataset2'},
        ],
        value = 'dataset1', #default selection
        clearable = False
    ),
...
# Define callback function to upgrade figure
...
def update_graph(selected_dataset, a, b, c, d):
    """Updates the graph based on the selected dataset."""

    # Define the data for different datasets
